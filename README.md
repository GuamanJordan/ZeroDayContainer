...
- [x] **Día 22:** Mapeo y reenvío de puertos mediante DNAT con `iptables` (`-p, --port`).
...
# 4. Con reenvío de puertos host -> contenedor (-p 8080:80)
sudo ./mc run --nat -p 8080:80 /tmp/alpine-rootfs /bin/sh

# 5. Modo de máxima seguridad con rootfs de solo lectura
sudo ./mc run --read-only /tmp/alpine-rootfs /bin/sh

# 6. Con límites de recursos garantizados por Cgroups v2
sudo ./mc run --memory=128m --cpus=1.0 --pids=50 /tmp/alpine-rootfs /bin/sh

# 7. Con pila de red aislada y veth pair
sudo ./mc run --net /tmp/alpine-rootfs /bin/sh

# 8. Con salida completa a Internet vía bridge mc0 y NAT
sudo ./mc run --nat /tmp/alpine-rootfs /bin/sh

# 9. Combinación completa para entorno de producción
sudo ./mc run --overlay --nat -p 8080:80 -v /home/user/app:/app --memory=256m --cpus=1.0 --pids=100 /tmp/alpine-rootfs /bin/sh
```
